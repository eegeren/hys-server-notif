...
DISABLE_GLOBAL_PROBE = os.getenv("DISABLE_GLOBAL_PROBE", "0") == "1"
...
WEEKLY_H  = int(os.getenv("WEEKLY_REPORT_WEEKDAY", "9"))
...
def send_raw(msg_html: str):
    """
    Telegram'a doğrudan gönderen fonksiyon.
    - DRY_RUN=1 ise sadece konsola yazar.
    - DEBUG modunda gelen HTTP status & body snippet'i yazdırır.
    """
    if DRY_RUN:
        if DEBUG:
            print(f"[TG-DRYRUN] {msg_html[:200].replace(chr(10),' ')}...")
        else:
            print(f"[TG-DRYRUN] mesaj (kısaltıldı): {msg_html[:80].replace(chr(10),' ')}...")
        return None
    try:
        r = SESSION.post(
            API_URL,
        ...
        if DEBUG:
            ...
            print(f"[TG-HTTP] status={getattr(r,'status_code',None)} body={body[:800]}")
        return r
...
def global_probe_ok():
    """
    If DISABLE_GLOBAL_PROBE is set, skip the probe (useful for tests).
    Otherwise run the ping probe as before.
    """
    if DISABLE_GLOBAL_PROBE:
        if DEBUG: print("[GLOBAL_PROBE] disabled via DISABLE_GLOBAL_PROBE")
        return True
    try:
        ok, info = check_ping("ping://" + GLOBAL_PROBE_TARGET)
        if DEBUG: print(f"[GLOBAL_PROBE] {GLOBAL_PROBE_TARGET} -> {ok} ({info})")
        return ok
    except Exception as e:
        if DEBUG: print(f"[GLOBAL_PROBE] error: {e}")
        return False
...
def main():
    ...
    # --- zorunlu self-test (rate limiter'i atlar, doğrudan gönderim) ---
    try:
        send_raw("🧪 SELFTEST: send_raw doğrudan test (force) — eğer gelmezse token/chat_id veya ağ sorunu var.")
    except Exception as e:
        if DEBUG: print(f"[SELFTEST] hata: {e}")
    ...
    while True:
        ...
        # debug: rate limiter durumunu göster
        if DEBUG:
            with rate_lock:
                try:
                    print(f"[RATE] window={len(rate_window)} queue={len(rate_queue)}")
                    if len(rate_queue) > 0:
                        for i, q in enumerate(rate_queue[:5]):
                            print(f"[RATE-QUEUE-{i}] {q[:200].replace(chr(10),' ')}")
                except Exception as e:
                    print(f"[RATE-DBG-ERR] {e}")
        ...
        save_state(state)
        if DEBUG:
            alive_supp = max(0, int(state.get('global_suppress_until', 0) - time.time()))
            alive_storm = max(0, int(state.get('storm_until', 0) - time.time()))
            print(f"[DEBUG] Tur süresi: {round(time.time()-loop_start,2)} sn | suppress={alive_supp}s | storm={alive_storm}s | hedef={len(items)}")
        time.sleep(INTERVAL)
